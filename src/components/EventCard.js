...
const EventCard = ({ event: { free, day, month, location, time, title, expectingPeopleMoreThan, mainHostImage, hostsImages, image } }) => {
  return (
    <StyledArticle $image={image}>
      ...
      {hostsImages.map((host, i) => (
        <CardAvatarSecondary key={i} $img={host} />
      ))}
      <CardAvatarMain $img={mainHostImage} />
      <CardContent>
        <CardSubtitleOne>
          <img style={{ width: '10px', marginRight: '4px' }} src={LocationIcon} alt="Location Icon"/>
